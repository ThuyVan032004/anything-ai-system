...
class EADagBuildConfigs:
    EA_ETL_DAG_BUILD_CONFIGS = DagETLBuildModel(
        ...
        clean_task=DagDockerTaskModel(
            ...
            image="emotion_analysis/data_pipelines/etl_pipeline:latest",
    ...
    EA_TRAINING_DAG_BUILD_CONFIGS = DagModelTrainingBuildModel(
        data_preparation_task=DagDockerTaskModel(
            task_id="ea_data_preparation_task",
            image="emotion_analysis/training_pipelines:latest",
            command="/bin/bash -c 'python -m emotion_analysis.models.ea_data_preparation'"
        ),
        model_training_task=DagDockerTaskModel(
            task_id="ea_model_training_task",
            image="emotion_analysis/training_pipelines:latest",
            command="/bin/bash -c 'python -m emotion_analysis.models.ea_model_training'"
        ),
        model_evaluation_task=DagDockerTaskModel(
            task_id="ea_model_evaluation_task",
            image="emotion_analysis/training_pipelines:latest",
            command="/bin/bash -c 'python -m emotion_analysis.models.ea_model_evaluation'"
        ),
        model_validation_task=DagDockerTaskModel(
            task_id="ea_model_validation_task",
            image="emotion_analysis/training_pipelines:latest",
            command="/bin/bash -c 'python -m emotion_analysis.models.ea_model_validation'"
        )
    )
